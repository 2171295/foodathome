...
class LoginController extends Controller
{
	...
	try {
            ...
            switch ($errorCode) {
                case '200':
                    Log::channel('cyber')->info("LOGIN SUCCESSFULL - Email: '" . $request->email . "'");
                    return json_decode((string)$response->getBody(), true);
                    break;
                case '301':
                    Log::channel('cyber')->info("LOGIN ATTEMPT - BLOCKED USER - Email: '" . $request->email . "'");
                    return response()->json(
                        ['msg' => 'Your account was blocked.', 'errorCode' => $errorCode]);
                    break;
                case '302':
                    Log::channel('cyber')->info("LOGIN ATTEMPT - DELETED USER - Email: '" . $request->email . "'");
                    return response()->json(
                        ['msg' => 'Your account was deleted.', 'errorCode' => $errorCode]);
                    break;
                default:
                    Log::channel('cyber')->info("LOGIN ATTEMPT - WRONG CREDENTIALS - Email: '" . $request->email . "'");
                    return response()->json(
